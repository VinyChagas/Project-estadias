...
if (isset($_SESSION['id'])) {
    ...
    $datainicio = $_POST['date-in'];
    $datasaida = $_POST['date-out'];
    $valorFinal = $_POST['valorFInal'];
    $taxa = $_POST['taxa'];
    $price_day = $_POST['price_day'];
    $limpeza = $_POST['taxaLimpeza'];
    $rezerva = $_POST['rezerva'];
    $quantie = $_POST['quantidade_dias'];
    $preco_sem_taxa = $_POST['preco_sem_taxa'];

    $data = array(
        'id_user' => $_SESSION['id'], // Substitua por valores reais
        ...
        'order_id' => 1, // Substitua por valores reais
...
?>
    ...
    <head>
        ...
        <link rel="stylesheet" href="css/style_payament.css">
    </head>
    ...
    <body>
        <div class='checkout'>
            <div class='order'>
                ...
                <h5>Seu pedido</h5>
                <ul class='order-list'>
                    <li><img src='https://png.pngtree.com/png-vector/20220808/ourmid/pngtree-3d-date-icon-png-image_6104303.png'>
                        <h4>E - <?php echo $datainicio; ?> <br> S - <?php echo $datasaida; ?></h4>
                        <h5 style="color: #3ac569;">R$ <?php echo $preco_sem_taxa; ?></h5>
                    </li>
                    <li><img src='https://cdn.icon-icons.com/icons2/3706/PNG/512/information_help_customer_service_support_technical_icon_229995.png'>
                        <h4>Taxa De serviço</h4>
                        <h5 style="color: #3ac569;">R$ <?php echo $taxa; ?></h5>
                    </li>
                    <li><img src='https://cdn-icons-png.flaticon.com/512/1669/1669005.png'>
                        <h4>Taxa De Limpeza</h4>
                        <h5 style="color: #3ac569;">R$ <?php echo $limpeza; ?></h5>
                    </li>
                </ul>

                <h5 class='total'>Total</h5>
                <h1>R$ <?php echo $valorFinal; ?></h1>
                <p style="font-size: 10px;">Ao pagar, você concorda com os termos de <a href="#"> POLITICAS DE CANCELAMENTO </a> e em respeitar as regras que o anfitrião definiu para o estabelecimento.</p>
            </div>
            <h2>Pagamento</h2>
            <div id='payment' class='payment'>
                ...
                <form method="POST" action="Confirmar_Pagamento/index.php" class='card-form'>
                    <p class='field'>
                    ...
                    <!-- Antes do botão, insira inputs ocultos para cada item de dados que você deseja enviar -->
                    <input type='hidden' id="id_user" name='id_user' value='<?php echo $_SESSION['id'] ?>' />
                    <input type='hidden' id="id_stay" name='id_stay' value='<?php echo $idStay; ?>' />
                    <input type='hidden' name='date_entrada' value='<?php echo $datainicio; ?>' />
                    <input type='hidden' name='date_saida' value='<?php echo $datasaida; ?>' />
                    <input type='hidden' name='price_day' value='<?php echo $price_day; ?>' />
                    <input type='hidden' name='price_total' value='<?php echo $valorFinal; ?>' />
                    <input type='hidden' name='order_id' value='1' />
                    <input type='hidden' name='taxa_limpeza' value='<?php echo $limpeza; ?>' />
                    <input type='hidden' name='taxa_servico' value='<?php echo $taxa; ?>' />

                    <button class='button-cta' title='Confirmar Pagamento' onclick="verifyPayament()" id="finalizar"><span>Reservar</span></button>
                </form>
            </div>
        </div>
        ...
        <script src="js/jsReservar.js"></script>
    </body>
